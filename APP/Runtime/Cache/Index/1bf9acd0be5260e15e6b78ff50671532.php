...
<!--顶部开始-->
	...
	<span style="font-size:14px;text-align:center;color:red;margin-left:0%">温馨提示：&nbsp;&nbsp;&nbsp;支付宝账户名即为你支付宝实名认证的真实姓名，如果审核发现支付宝没有实名认证或者姓名不对应将无法通过认证！</span>
		...
		<?php if(!empty($memmber['liyou']) and $memmber['checkstatus'] == 2): ?><li style="height: 30px;line-height: 30px;margin-top: 5px;width:100%">审核结果：&nbsp;&nbsp;&nbsp;<font style="color:#F00;"><?php echo ($memmber['liyou']); ?></font></li>
       		<button type="submit" class="btn btn-primary btn-submit btn_submit" style="width: 100%;height: 30px;line-height: 30px;border-radius: 5px;border: 0px; background-color:#FFA500;margin-top: 5px;color: #FFFFFF;-webkit-appearance: none;">提 交</button>
       
       <?php else: ?>
       	
            <?php if(empty($memmber['weixin']) || empty($memmber['truename'])): ?><button type="submit" class="btn btn-primary btn-submit btn_submit" style="width: 100%;height: 30px;line-height: 30px;border-radius: 5px;border: 0px; background-color:#FFA500;margin-top: 5px;color: #FFFFFF;-webkit-appearance: none;">提 交</button><?php endif; endif; ?>	
