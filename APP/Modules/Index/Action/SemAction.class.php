...
class SemAction extends Action {
			...
			foreach ( $orderlist as $key => $val ) {
				...
				if ($val ['count'] >= $val ['yxzq']) { // 判断如果相差时间等于720小时
					...
					foreach ( $parentpath as $m => $n ) {
						$member->where ( array (
								'id' => $n 
						) )->setDec ( "teamgonglv", $val ['lixi'] );
					}
					// 扣除个人算力
					$member->where ( array (
							'username' => $username 
					) )->setDec ( "mygonglv", $val ['lixi'] );
					...
					if (! $ac && $cha >= 24) {
						...
						foreach ( $parentpath as $m => $n ) {
							...
							$ldgroup = M ( "member_group" )->where ( array (
	...
	public function regSempost() {
		if (IS_AJAX) {
			...
			$data ['parent'] = $parent;
			$data ['email'] = I ( 'post.email', '', 'strval' );
			
			$data ['parent_id'] = M ( 'member' )->where ( array (
					'username' => $parent
			) )->getField ( 'id' );
...
// 			$data ['weixin'] = I ( 'post.weixin', '', 'strval' );
			$data ['zhifubao'] = I ( 'post.mobile', '', 'strval' );
...
// 			}
			...
			$check_code = email_code_verify($data['email'],$data['mobile'],$code,session_id());
			...
			$parentinfo = M ( 'member' )->where ( array (
					'username' => $data ['parent'] 
			) )->find ();
			...
			M ( 'member' )->where ( array (
					'username' => $data ['parent'] 
			) )->setInc ( 'parentcount', 1 );
			mmtjrennumadd ( $parent_id ); // 所有上级加一人
			
			$this->ajaxReturn ( array (
					'result' => 1,
					'info' => '注册成功！请登录后完善个人资料!' 
			) );
		}
	}
	...
	public function checkParent() {
		...
		// 是否激活noactivation
		if (! $nickname = $member->where ( array (
				'username' => $_POST ['username'],
				'status' => 1 
		) )->getField ( 'nickname' )) {
	 ...
	 * 发送邮件注册验证码
	 */
	public function send_mail_reg_code() {
		$mobile= I ( 'mobile' );
		...
		if (! check_mobile ( $mobile))
			...
			$code = rand ( 1000, 9999 );
			$send = mail_log ( $mobile, $email, $code, session_id () );
			if ($send ['status'] != 1) {
				exit ( json_encode ( array (
						'status' => - 1,
// 						'msg' => $send ['msg']
	...
	public function notify() {
		...
		if ($sign_md5 == $sign) {
			...
			M ( "member_recharge" )->where ( "order_sn = '{$orderid}' and status = 0" )->save ( array (
					'status' => 1 
			) );
