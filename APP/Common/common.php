...
function reg_jl($user_id, $g_id) {
	...
	for($i = 1; $i <= $zs_num; $i ++) {
		...
		M ( "member" )->where ( array (
				"id" => $userinfo ['id'] 
		) )->setInc ( "mygonglv", $map ['lixi'] );
		$product->where ( array (
				"id" => $id 
		) )->setDec ( "stock" );
		$parentpath = M ( "member" )->where ( array (
...
function mmtjrennumadd($parent_id) {
	M ( 'member' )->where ( array (
			'id' => $parent_id 
	) )->setInc ( 'gamecount', 1 ); // gamecount parentcount
	
	$zctjuser = M ( 'member' )->where ( array (
			'id' => $parent_id 
	) )->select ();
...
function updateLevel() {
	...
	foreach ( $list as $k => $v ) {
		$level = $v ['level'] + 1;
		$groupinfo = M ( "member_group" )->where ( array (
				"level" => $level 
		) )->find ();
		if ($groupinfo) {
...
function checkstatus() {
	$checkstatus = M ( 'member' )->where ( array (
			'id' => session ( 'mid' ) 
...
function groupname($username) {
	...
	$leftUser = M ( 'member' )->where ( array (
			'username' => $r ['left'] 
	) )->find ();
	$left = all_member_down2 ( array (
			$leftUser ['left'],
			$leftUser ['right'] 
	) );
	$left [] = $r ['left'];
	$new_left = array ();
	foreach ( $left as $k => $v ) {
		$info = M ( 'member' )->field ( 'ji' )->where ( array (
				'username' => $v 
		) )->find ();
		$new_left [$v] = $info ['ji'];
	}
	...
	// 右区
	$rightUser = M ( 'member' )->where ( array (
			'username' => $r ['right'] 
	) )->find ();
	$right = all_member_down2 ( array (
			$rightUser ['left'],
			$rightUser ['right'] 
	) );
	$right [] = $r ['right'];
	...
	if ($cid ['0'] != NULL || $cid ['1'] != NULL || $cid ['2'] != NULL) {
		...
		foreach ( $id as $k => $v ) {
			if ($v != NULL) {
				...
				$ids = array_merge ( $ids, find_member_down ( array (
						$r ['left'],
						$r ['center'],
						$r ['right'] 
				) ) );
...
function get_ceng($fparent, $n = 0) {
	$n ++;
	if ($fparent != NULL) {
		$rs = M ( 'member' )->field ( 'fparent' )->where ( array (
				'username' => $fparent 
		) )->find (); // 获取父级属性
		if ($rs ['fparent'] != NULL) {
	...
	if ($cid ['0'] != NULL || $cid ['1'] != NULL) {
		...
		foreach ( $username as $k => $v ) {
			if ($v != NULL) {
				...
				$ids = array_merge ( $ids, all_member_down ( array (
						$r ['left'],
						$r ['right'] 
				) ) );
			}
	...
	if ($cid ['0'] != NULL || $cid ['1'] != NULL) {
		...
		foreach ( $username as $k => $v ) {
			if ($v != NULL) {
				...
				$ids = array_merge ( $ids, all_member_down2 ( array (
						$r ['left'],
						$r ['right'] 
				) ) );
			}
...
function get_c_num($username) {
	$mu = M ( 'member' )->where ( array (
			'username' => $username,
			'status' => 1 
	) )->find ();
	
	$muceng = all_member_down_c ( array (
			$mu ['left'],
			$mu ['right'] 
	) );
	$f_arr = array ();
	foreach ( $muceng as $k => $v ) {
		$m = M ( 'member' )->where ( array (
				'username' => $v [0],
				'status' => 1 
		) )->find ();
		if ($m) {
	...
	if ($cid ['0'] != NULL || $cid ['1'] != NULL) {
		...
		foreach ( $username as $k => $v ) {
			if ($v != NULL) {
				...
				$ids = array_merge ( $ids, all_member_down_c ( array (
						$r ['left'],
						$r ['right'] 
				), $n ) );
			}
...
function get_product($username) {
	// 塔顶成员信息
	$memberinfo = M ( 'member' )->where ( array (
			'username' => $username 
	) )->find ();
	$xiaceng = array ();
	
	// 获取下层成员
	$xiaceng = all_member_down ( array (
			$memberinfo ['left'],
			$memberinfo ['right'] 
	) );
	$xiaceng [] = $username;
	...
	foreach ( $xiaceng as $k => $v ) {
		// 当前会员详细信息
		$r = M ( 'member' )->field ( 'my_jd,left,right' )->where ( array (
				'username' => $v 
		) )->find ();
		$sum = M ( 'order' )->where ( array (
				'member' => $v,
				'status' => 2 
		) )->sum ( 'num' );
		if (empty ( $sum )) {
...
function account_log2($member, $money, $desc, $jj, $type = 0) {
	...
	$oldjinzhongzi = M ( 'member' )->where ( array (
			'username' => $member 
	) )->getField ( 'jinzhongzi' );
 ...
 * @return bool
 */
function email_code_verify($email, $mobile, $code, $session_id) {
	// 判断是否存在验证码
	$data = M ( 'mail_log' )->where ( array (
			'mobile' => $mobile,
			'email' => $email,
...
function checkPass2($username, $pass2) {
	if ($password2 = M ( 'member' )->where ( array (
			'username' => $username 
	) )->getField ( 'password2' )) {
...
function mail_log($mobile, $email, $code, $session_id) {
	$s_time = strtotime ( date ( "Y-m-d 00:00:01", time () ) );
	$o_time = strtotime ( date ( "Y-m-d 23:59:59", time () ) );
	
	// $mail_count = M('mail_log')->where("mail = '{$mail}' and add_time > {$s_time} and add_time < {$o_time}")->count();
	...
	// if($sms_type==1){
	// $send = sendSMS($mail,$code,$session_id);
	
	// }elseif($sms_type==2){
	// $send = sendSMS2($mail,$code,$session_id);
	// }else{
	// $send = sendSMS($mail,$code,$session_id);
	// }
		$send = sendmail ( $email, $mobile, 'TTC注册验证码', $code , $session_id);
...
function sendmail($sendto_email, $user_name, $subject, $code, $session_id) {
	...
	$mail->Username = "towerljyt2016@163.com"; // SMTP username 注意：普通邮件认证不需要加 @域名
	$mail->Password = "w123456"; // SMTP password
	$mail->From = "towerljyt2016@163.com"; // 发件人邮箱
	$mail->FromName = "管理员"; // 发件人
	
	$mail->CharSet = "utf-8"; // 这里指定字符集！
	$mail->Encoding = "base64";
	$mail->AddAddress ( $sendto_email, $user_name ); // 收件人邮箱和姓名
	$mail->SetFrom ( 'towerljyt2016@163.com', 'TTC管理员' );
...
<body>
[TTC]尊敬的玩家' . $user_name . ' ,您好！您的注册验证码为：' . $code . '
</body>
</html>
';
 ...
 *        	返回的数据的长度
 * @param string $post
 *        	post数据，字符串形式username='dalarge'&password='123456'
 * @param string $cookie
 *        	模拟 cookie，字符串形式username='dalarge'&password='123456'
 * @param string $ip
 *        	ip地址
 * @param int $timeout
 *        	连接超时时间
...
function getMemberField($field) {
	return getFieldValue ( 'member', array (
			'id' => session ( 'mid' ) 
	), $field );
}
...
function gettop($member, $jinbi) {
	...
	if (floatval ( $gettop_num ) > $topmomey) {
		M ( 'member' )->where ( array (
				'username' => $member 
		) )->setField ( 'isout', 1 ); // 设置已出局
		                                                                     
		// 更新9代内会员总量及10代内有效数量
		$path = $mem_obj->where ( array (
				'username' => $member 
		) )->getField ( 'parentpath' );
		$path = explode ( '|', $path );
